...
* **雙端自適應介面**：
  * **桌面端**：左右雙欄結構（`發送 ｜ 接收`），提供圖片大圖預覽、Finder 檔案位置捷徑與常駐連線 QR Code。
  * **手機端**：上下單欄結構（`發送 ↓ 接收`），自動隱藏 QR Code 與本機 Finder 操作按鈕，介面簡潔俐落。
...
2. **傳送圖片**：
   ...
   * 點擊「送圖片到 Mac」。
   * 切換至 Mac 任意應用程式直接按 `Cmd + V` 貼上圖片。
3. **傳送文字**：於文字框貼上文字後點擊「送文字到 Mac」，Mac 直接按 `Cmd + V` 貼上。
